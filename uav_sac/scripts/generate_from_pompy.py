...
gas_masks = np.zeros((frames, xdim, ydim))
for i in range(frames):
    gas_map = np.zeros((xdim, ydim))
    gas_map = update(i)
    gas_masks[i] = np.ma.masked_where(gas_map > 1e6, gas_map).mask.astype("half")
    gas_masks[i][gas_masks[i] == 1.] = 0.98
    gas_masks[i][gas_masks[i] == 0.] = 0.02
    wind_speeds[i] = wind_model.velocity_field
map_over_time = np.concatenate((wind_speeds, gas_masks.reshape(frames, xdim, ydim, 1)), axis=3)
